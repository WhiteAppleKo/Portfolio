...
<p align="center">
  <b>게임 Portal의 핵심 포탈 물리 및 실시간 다방향 뷰 렌더링 메커니즘을 R&D한 1인 3D 모작 프로젝트</b>
</p>
...
### 1.4. 시스템 아키텍처 구조도
아래 아키텍처 다이어그램은 플레이어 카메라 시점이 포탈을 통해 변환 행렬을 거쳐 반대편 가상 카메라로 투영되고, 최종 클립 셰이더 단면 처리를 거쳐 화면에 출력되는 실시간 제어 흐름을 도식화한 것입니다.

```mermaid
flowchart TD
    PlayerCam["Main Camera<br>(Player View)"] --> CheckVisible{"Frustum Culling Check<br>(VisibleFromCamera)"}
    CheckVisible -- No: Rendering Skip --> Skip["Render Process Terminated"]
    CheckVisible -- Yes: Trigger Render --> CalcMatrix["Matrix4x4 Transform Matrix<br>(WorldToLocal * LocalToWorld)"]
    CalcMatrix --> UpdatePortalCam["Portal Camera Position/Rotation Updated"]
    UpdatePortalCam --> CreateClone["Spawn GraphicsClone<br>(Mirror Copy Object)"]
    CreateClone --> ClipShader["Custom Clip Shader<br>(clip negative Dot Product)"]
    ClipShader --> RenderTex["RenderTexture Generated"]
    RenderTex --> Screen["Portal Mesh Screen Output"]
```
