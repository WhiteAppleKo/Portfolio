...
<!-- link-github: https://github.com/WhiteAppleKo/ChickenChaCha -->
<!-- link-video: https://youtu.be/rRhMZpOV2sA -->
...
### 1.1. 프로젝트 정의
* **프로젝트 정의**: 전통 보드게임 '치킨 차차'의 규칙을 멀티플레이 환경으로 디지털 이식한 3D 온라인 멀티플레이 팀 프로젝트입니다.
* **핵심 구현**: Photon Fusion 2 상태 동기화 기반 턴 제어 시스템 및 예기치 않은 네트워크 단절 대응 세션 유지 프레임워크 구축을 구현했습니다.
...
```
### 1.2. R&D 동기 및 협업 배경
* **R&D 동기 및 배경**: 팀 단위 작업이 처음인 팀원들과 원활한 개발 진행 및 방장을 포함한 임의의 플레이어의 네트워크 이탈 시에도 게임 세션이 중단 없이 안전하게 복구 및 동기화되는 프레임워크 수립을 위해 진행되었습니다.
* **문서의 기술 범위**: 본 문서는 UI 시스템에 국한되지 않고, 실시간 네트워크 환경에서 방장을 포함한 임의의 플레이어 이탈 시에도 중단 없는 플레이를 보장하기 위해 설계된 멀티플레이어 상태 동기화를 소개합니다.
...
| **05. 고민과 선택 : 대안 비교 및 결정 근거** | 개발 중 발생한 설계 절충안 분석            | 데이터 패킷 전송 모델, 협업 마일스톤 비교 결정 |
...
### 6.2. 한계점 및 배운 점
* **달성한 성과 및 한계 (✓ / △ / →)**:
  * **✓ 달성한 성과**: 5인 팀 프로젝트의 기술 리드로서 인터페이스 명세 기반 분할 작업 프로세스를 도입하여 병목 현상 없이 병렬 개발을 수행하였고, Shared Mode를 채택하여 마스터 클라이언트 이탈 시에도 세션 유지 및 예외 복구를 구현했습니다.
  * **△ 한계점**: 초기 개발 기획 시 너무 세세하게 작업 계획을 칸반 보드에 작성하여 개발자의 자율성이 다소 제한되고 일정 지연이 유발되었습니다.
  * **→ 향후 계획**: 차기 프로젝트 진행 시에는 세세한 칸반 계획 수립 방식을 지양하고, 초기 인터페이스 규격 및 명세 정의에 집중하여 병렬 독립 테스트 환경을 견고히 유지할 예정입니다.
